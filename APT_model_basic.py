...
class MyProtfolio:
    ...
    def __init__(self,NumOfAsset,AssetList,Factor,Return,start_date,end_date):
        ...
        for i in range(NumOfAsset):
            ...
            self.DataSet.append(get_factor(self.AssetList[i],Factor,start_date=start_date,end_date=end_date))
        print('Return=',Return)
        self.Return=get_factor(AssetList,Return,start_date=start_date,end_date=end_date)
    ...
    def cal_coe(self,df,i,j):
        #print(df[[self.Factor[i],self.Factor[j]]])
    ...
    def CalRiskExpose(self):
        ...
        for i in range(self.num_asset()):
            ...
            delta=row
            Y=self.Return[i*delta:(i+1)*delta]
            model=LinearRegression()
            model.fit(X,Y)
            W=model.coef_
            for j in range(self.num_factor()):
                self.RiskExpose[i][j]=W[0][j]
...
'''
Factor=[#'cash_received_from_sales_of_goods','cash_from_operating_activities','begin_period_cash_equivalent',# from cash flow
        #'financial_asset_held_for_trading','cash_equivalent','total_fixed_assets','total_equity_and_liabilities',#from balance sheet
